...
import { getlevelCSSColor, getlevelSize } from '../../services/utils';
  ...
  set markers({ maskOption, data }: { maskOption: number; data: any[] }) {
    if (!!data) {
      ...
      this._markers = data.map(x => {
        ...
        const markerColor = getlevelCSSColor(level);
        const size = getlevelSize(level);
        const iconUrl = `https://cdn.mapmarker.io/api/v1/pin?icon=fa-star&size=${size}&background=${markerColor}`;
        return {
          id: x.id,
          position: {
            lng: x.longitude,
