...
export class AppComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    combineLatest([
      this.displayOption.valueChanges.pipe(startWith(10)),
      this.maskOption.valueChanges.pipe(startWith(1))
    ])
      .pipe(
        map(([display, mask]) => {
          this.displayOptionDisplay = this.pageOptions.find(
            x => x.value === display
          ).display;
          this.maskTypesDisplay = this.masktypes.find(
            x => x.value === mask
          ).display;
        })
      )
